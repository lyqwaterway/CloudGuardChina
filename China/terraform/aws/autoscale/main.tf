...
resource "aws_security_group" "permissive_sg" {
  name_prefix = format("%s_PermissiveSecurityGroup", local.asg_name_local)
  ...
  tags = {
    Name = format("%s_PermissiveSecurityGroup", local.asg_name_local)
  }
}

resource "aws_launch_template" "asg_launch_template" {
  name_prefix = local.asg_name_local
...
resource "aws_iam_role" "role" {
  ...
  name_prefix = format("%s-iam_role", local.asg_name_local)
...
module "attach_cloudwatch_policy" {
  ...
  tag_name = local.asg_name_local
}

resource "aws_iam_instance_profile" "instance_profile" {
  count = local.create_iam_role
  name_prefix = format("%s-iam_instance_profile", local.asg_name_local)
  path = "/"
  role = aws_iam_role.role[count.index].name
}
